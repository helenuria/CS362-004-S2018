...
int council_room_card(struct gameState *state, int handPos)
{
	int currentPlayer = whoseTurn(state);
	int i = 0;
	//+4 Cards
...
int remodel_card(struct gameState *state, int handPos, int choice1, int choice2)
{
	int currentPlayer = whoseTurn(state);
	int i = 0;
	int j = 0;
...
int village_card(struct gameState *state, int handPos)
{
	int currentPlayer = whoseTurn(state);
	//+1 Card
	drawCard(currentPlayer, state);
...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
	...
	{
	case adventurer:
		return adventurer_card(state);

	case council_room:
		return council_room_card(state, handPos);
	...
	case mine:
		return mine_card(state, handPos, choice1, choice2);

	case remodel:
		return remodel_card(state, handPos, choice1, choice2);

	case smithy:
		return smithy_card(state, handPos);

	case village:
		return village_card(state, handPos);
